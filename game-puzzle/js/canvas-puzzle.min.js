/* Copyright 2012 Jhonathan Salguero Villa <http://www.novatoz.com>
 ...
 */
 (function() {
    ...
    jigsaw.Jigsaw = Class.extend({
        init: function(l, k) {
            ...
            function i() {
                ...
                m.width = n.width = document.documentElement.clientWidth,
                m.height = n.height = document.documentElement.clientHeight + 20;
                j.ce.redraw()
            }
            i();
    ...
    function d(i, h) {
        ...
        i.cv.addEventListener("contextmenu",
        function(j) {
            if (i.focused) {
                i.focused.rotation = (i.focused.rotation + 45) % 360;
                i.redraw()
    ...
    jigsaw.UI = Class.extend({
        init: function(d) {
			...
			//游戏完成时要执行的方法，停止计时器并显示游戏完成时间。
            d.on(jigsaw.Events.JIGSAW_COMPLETE,
            function() {
                e.stop_clock();
                e.show_time();
				//游戏完成时在这里添加其他方法。
        ...
        init_clock: function() {
            ...
            var d1 = c("image-preview");
            d1.className = d1.className == "show" ? "show": "show";
			
			//这里的背景参考图初始值是显示状态时定位会有问题，故而注释掉以下代码；
            d1.style.marginLeft = -(d1.width / 2) + "px"
        },
        show_preview: function() {
            var d = c("image-preview");
            d.className = d.className == "show" ? "show": "show";
        ...
        show_time: function() {
			...
			//游戏完成时所显示的完成时间在这里读取！
            c("typeFun").value = "1";
            c("secTime").value = this.clock.innerHTML;
