...
(function($)
{
	...
	{
		width			: null,					// set width - best to match image width
		height			: null,					// set height - best to match image height
		image			: null,	// set image path
		image2			: null,					// set overlay image path - if set color is not used
		color			: null,			// set scratch color - if image2 is not set uses color
		overlay			: 'none',				// set the type of overlay effect 'none', 'lighter' - only used with color
		size			: null,					// set size of scratcher
	...
	ScratchPad.prototype = 
	{
		...
		init: function()
		{
			...
			this.sp.css({backgroundSize: '100% 100%'});
		...
		drawImage: function(imagePath)
		{
			...
			var img = new Image();
  			...
  			$(img).load(function(){
  				$this.ctx.drawImage(img,0,0,oWidth,oHeight);
				$this.setBgImage();
  			})
		},
